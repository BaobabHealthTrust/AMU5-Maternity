class EncountersController < ApplicationController
  ...
  def create(params=params, session=session)
    ...
    @lmp = Observation.find(:all, :conditions => ["concept_id IN (?) AND person_id = ? AND encounter_id IN (?)", 
        ConceptName.find_by_name("LAST MENSTRUAL PERIOD").concept_id, @patient.person.id, @patient.encounters.collect{|e| e.id}],
  ...
  def observations_printable
    ...
    @MaternityPatient.create_barcode
